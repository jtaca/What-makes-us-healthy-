...
const RadarChart = function RadarChart(parent_selector, data, options, tip7, onCreate, svg, colorLegend) {
	...
	let g = onCreate ? svg.append("g").attr("transform", "translate(" + (cfg.w / 2 + 50) + "," + (cfg.h / 3 + cfg.margin.top) + ")")
		: svg.select("g");
	...
	if (onCreate) {
		...
		blobCircleWrapper.selectAll(".radarInvisibleCircle")
			...
			.on("mouseover", function (d,i) {
				...
				d3.select(this).transition().duration(500).style("fill", colorLegend).attr("stroke-width", 1).attr("stroke", "black");
				tip7.transition()
					.duration(200)
					.style("opacity", 1);
				tip7.html(content = "<span style=\"display: block;background-color: rgb(0,0,0); opacity:0.7;color: white; text-shadow:none; padding:9px; top:0px\">"
					+ "<b>Correlation ("+ d.axis+"): </b>" + (d.value).toFixed(3) + "</span>")
			...
			.on("mouseout", function (d,i) {
				...
				d3.select(this).transition().duration(500).style("fill", "none").style("stroke-with",0).attr("stroke", "none");
				tip7.transition()
					.duration(500)
					.style("opacity", 0);
			});
