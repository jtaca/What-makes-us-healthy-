...
// Wheel component
//
var TheWheel = (function(window, undefined) {
  ...
  function addEventListeners() {
    if (refreshButtonEl) refreshButtonEl.addEventListener('click', api.render);
  }

  function removeEventListeners() {
    if (refreshButtonEl) refreshButtonEl.removeEventListener('click', api.render);
  }

  api.render = function() {
    svg.selectAll('g.single')
      .data([randomDataset()])
      .call(chart);
  }
  ...
  api.init = function() {
    ...
    chartContext = document.getElementById('js-chart-container');
    refreshButtonEl = document.getElementById('js-refresh');

    svg =
      d3
      .select(chartContext)
      .append('svg')
      .classed('u-svg-fluid', true)
